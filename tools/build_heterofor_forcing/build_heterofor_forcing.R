...
read_one_coverage <- function(coverage, varnames, path) {
    ...
    for (varname in varnames) {
        df[[varname]] <- unlist(
            coverage$ranges[[varname]]$values,
            use.names = FALSE
        )
    }
    df
...
read_coveragejson_long <- function(json_doc, varnames, path) {
    ...
    long_tables <- lapply(
        coverages, read_one_coverage,
        varnames = varnames, path = path
    )
...
write_station_files <- function(df, out_dir, header, suffix) {
    ...
    write_one <- function(df_station, station_name) {
        ...
        write.table(
            df_station,
            file = file_name, sep = "\t", row.names = FALSE,
...
main <- function() {
    ...
    if (!is.null(opt$reneco_rds)) {
        ...
        if (!is.null(opt$filling_summary)) {
            write.table(
                combined$filling_summary,
                file = opt$filling_summary, sep = "\t",
                row.names = FALSE, na = "NA", dec = "."
            )
        }
        if (!is.null(opt$missing_summary)) {
            write.table(
                combined$missing_summary,
                file = opt$missing_summary, sep = "\t",
                row.names = FALSE, na = "NA", dec = "."
            )
        }
